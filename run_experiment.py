...
if (mode == "all") or (mode == "train"):
    ...
    experiment_params = {
        name: param
        for name, param in inspect.signature(experiment_class.init_special).parameters.items()
        if name != "self"
    }
    ...
    p.add_argument(
        "--numpoints", type=int, default=65000, help="Number of points in simulation data source __getitem__."
    )
    p.add_argument(
        "--pretrain", action="store_true", default=False, required=False, help="Pretrain dirichlet conditions"
    )
    ...
    p.add_argument(
        "--pretrained_model", type=str, default="none", required=False, help="Whether to use pretrained model"
    )
    p.add_argument(
        "--finetune",
        action="store_true",
    ...
    p.add_argument(
        "--num_hl", type=int, default=3, required=False, help="The number of hidden layers"
    )  # Don't recommand tuning this
    p.add_argument(
        "--num_nl", type=int, default=512, required=False, help="Number of neurons per hidden layer."
    ...
    # MPC loss options
    p.add_argument("--not_use_MPC", default=False, action="store_true", help="use MPC loss")
    p.add_argument(
        "--not_refine_dataset", default=False, action="store_true", help="refine MPC dataset"
    )  # whether we refine the MPC dataset every H_R seconds (see paper)
    p.add_argument(
        "--MPC_finetune_lambda", type=float, default=100.0, help="MPC finetuning weight for False Positives"
    )  # working fine but tunable. can be lower if causing instability
    p.add_argument(
        "--num_MPC_data_samples", type=int, default=5000, help="Number of MPC data samples used for training"
    )  # working fine but tunable.
    p.add_argument(
        "--no_time_curr", default=False, action="store_true", help="use MPC loss with time curriculum"
    )  # Currently please always use --time_curr with --use_MPC
    # TODO: do a PDE weight curriculum if time_curr==False
    p.add_argument("--MPC_importance_init", type=float, default=1.0, help="importance of MPC loss at the beginning")
    p.add_argument("--MPC_importance_final", type=float, default=1.0, help="importance of MPC loss at the end")
    ...
    p.add_argument(
        "--num_MPC_perturbation_samples", type=int, default=100, help="Number of MPC samples"
    )  # working fine but tunable
    p.add_argument(
        "--num_iterative_refinement", type=int, default=20, help="Number of MPC iterative refinement steps"
    )  # working fine but tunable
    p.add_argument(
    ...
    p.add_argument(
        "--num_target_samples", type=int, default=0, required=False, help="Number of samples inside the target set"
    )
    ...
    p.add_argument("--resume_checkpoint", type=int, default=-1, required=False, help="The number of hidden layers")

    # training options
    p.add_argument("--steps_til_summary", type=int, default=100, help="epochs until summary is printed.")
    p.add_argument(
        "--batch_size",
        type=int,
        default=1,
    ...
    p.add_argument("--MPC_lambda_", type=float, default=0.1, help="MPPI lambda, which tunes the weighting strategy")
    ...
    p.add_argument("--aug_with_MPC_data", type=int, default=0, help="How many of pde samples are from the MPC traj")
    p.add_argument(
        "--MPC_decay_scheme",
        type=str,
        default="exponential",
    ...
    p.add_argument(
        "--val_x_resolution", type=int, default=200, help="x-axis resolution of validation plot during training"
    )
    p.add_argument(
        "--val_y_resolution", type=int, default=200, help="y-axis resolution of validation plot during training"
    )
    p.add_argument(
        "--val_z_resolution", type=int, default=5, help="z-axis resolution of validation plot during training"
    )
    p.add_argument(
...
if (mode == "all") or (mode == "test"):
    ...
    p.add_argument(
        "--gt_data_path", type=str, default="none", help="Folder for gt data where coords.pt and gt_values.pt exist"
    )
...
if orig_opt.pretrained_model != "none":
    model.load_state_dict(
        torch.load("./runs/%s/training/checkpoints/model_final.pth" % orig_opt.pretrained_model)["model"]
    )
...
experiment.init_special(
    **{
        argname: getattr(orig_opt, argname)
        for argname in inspect.signature(experiment_class.init_special).parameters.keys()
        if argname != "self"
    }
)
