...
class Experiment(ABC):
    ...
    def train(
        ...
        scheduler = torch.optim.lr_scheduler.ExponentialLR(
            self.optim, gamma=0.9997
        )  # by default running without scheduler, 0.9997
            ...
            for epoch in range(0, epochs):
                ...
                for step, (model_input, gt) in enumerate(train_dataloader):
                    ...
                    if len(model_input["MPC_inputs"].shape) > 2:  # When MPC data is available
                        MPC_results = self.model({"coords": model_input["MPC_inputs"]})
                    ...
                    for loss_name, loss in losses.items():
                        ...
                        writer.add_scalar(loss_name, single_loss * self.loss_weights[loss_name], total_steps)
                ...
                if not (epoch + 1) % epochs_til_checkpoint:
                    ...
                    np.savetxt(
                        os.path.join(checkpoints_dir, "train_losses_epoch_%04d.txt" % (epoch + 1)),
                        np.array(train_losses),
                    )
                    self.validate(
                        epoch=epoch + 1,
                        save_path=os.path.join(checkpoints_dir, "BRS_validation_plot_epoch_%04d.png" % (epoch + 1)),
                        x_resolution=val_x_resolution,
                        y_resolution=val_y_resolution,
    ...
    def test(
        ...
        if checkpoint_toload is None:
            ...
            for i in tqdm(range(sidelen), desc="Checkpoint"):
                ...
                for j in tqdm(range(sidelen), desc="Simulation Time", leave=False):
                    ...
                    results = scenario_optimization(
                        ...
                        scenario_batch_size=min(num_scenarios, 100000),
                        sample_batch_size=min(10 * num_scenarios, 1000000),
                        ...
                        max_samples=1000 * num_scenarios,
                    )
                    ...
                    if results["maxed_scenarios"]:
                        ...
                        BRT_error_region_fracs_matrix[i, j] = target_fraction(
                            ...
                            batch_size=min(10 * num_scenarios, 1000000),
                        )
                    else:
            ...
            # plot the matrices
            matrices = {
                "BRT_volumes_matrix": [BRT_volumes_matrix, "BRT Fractions of Test State Space"],
                "BRT_errors_matrix": [BRT_errors_matrix, "BRT Errors"],
                "BRT_error_rates_matrix": [BRT_error_rates_matrix, "BRT Error Rates"],
                "BRT_error_region_fracs_matrix": [BRT_error_region_fracs_matrix, "BRT Error Region Fractions"],
                "exBRT_volumes_matrix": [exBRT_volumes_matrix, "exBRT Fractions of Test State Space"],
                "exBRT_errors_matrix": [exBRT_errors_matrix, "exBRT Errors"],
                "exBRT_error_rates_matrix": [exBRT_error_rates_matrix, "exBRT Error Rates"],
                "exBRT_error_region_fracs_matrix": [exBRT_error_region_fracs_matrix, "exBRT Error Region Fractions"],
            }
            ...
            if data_step == "eval_w_gt":
                ...
                with torch.no_grad():
                    ...
                    pred_values = self.dataset.dynamics.io_to_value(
                        results["model_in"].detach(), results["model_out"].squeeze(dim=-1).detach()
                    )
                ...
                np.save(os.path.join(testing_dir, f"mse.npy"), mse.cpu().numpy())
                np.save(os.path.join(testing_dir, f"fp.npy"), torch.tensor([fp]))
                np.save(os.path.join(testing_dir, f"fn.npy"), torch.tensor([fn]))
                print("False positive: %0.4f, False negative: %0.4f" % (fp, fn))
                print("MSE: ", mse)
            ...
            if data_step == "plot_robust_recovery":
                ...
                fig, values_slices = self.plot_recovery_fig(dataset, dynamics, model, delta_level)
                fig.savefig(os.path.join(testing_dir, f"robust_BRTs_1e-2.png"), dpi=800)
            ...
            if data_step == "run_robust_recovery":
                ...
                logs["beta_"] = beta_
                logs["N"] = N
                delta_level = float("inf") if dynamics.set_mode in ["reach", "reach_avoid"] else float("-inf")
                ...
                results = scenario_optimization(
                    ...
                    set_type=set_type,
                    control_type=control_type,
                    scenario_batch_size=min(N, 100000),
                    sample_batch_size=10 * min(N, 10000),
                    sample_generator=SliceSampleGenerator(dynamics=dynamics, slices=[None] * dynamics.state_dim),
                    sample_validator=ValueThresholdValidator(v_min=float("-inf"), v_max=delta_level)
                    if dynamics.set_mode in ["reach", "reach_avoid"]
                    else ValueThresholdValidator(v_min=delta_level, v_max=float("inf")),
                    violation_validator=ValueThresholdValidator(v_min=0.0, v_max=float("inf"))
                    if dynamics.set_mode in ["reach", "reach_avoid"]
                    else ValueThresholdValidator(v_min=float("-inf"), v_max=0.0),
                    max_scenarios=N,
                    max_samples=1000 * min(N, 10000),
                )

                sns.set_style("whitegrid")
                costs_ = results["costs"].cpu().numpy()
                values_ = results["values"].cpu().numpy()
                unsafe_cost_safe_value_indeces = np.argwhere(np.logical_and(costs_ < 0, values_ >= 0))
                ...
                for delta_level_ in np.arange(0, delta_level_max, delta_level_max / 100):
                    k = int(np.argwhere(np.logical_and(costs_ < 0, values_ >= delta_level_)).shape[0])
                ...
                fig1.savefig(os.path.join(testing_dir, f"robust_verification_results.png"), dpi=800)
                plt.close(fig1)
                np.save(os.path.join(testing_dir, f"epsilons"), epsilons)
                np.save(os.path.join(testing_dir, f"volumes"), volumes)
                np.save(os.path.join(testing_dir, f"deltas"), np.arange(0, delta_level_max, delta_level_max / 100))
                np.save(os.path.join(testing_dir, f"ks"), ks)
            ...
            if data_step == "run_basic_recovery":
                ...
                for i in range(M):
                    ...
                    algorithm_iters.append(
                        {"states": results["states"], "values": results["values"], "violations": results["violations"]}
                    )
                    if results["violation_rate"] == 0:
                        break
                    violation_levels = results["values"][results["violations"]]
                    delta_level_arg = (
                        np.argmin(violation_levels)
                        if dynamics.set_mode in ["reach", "reach_avoid"]
                        else np.argmax(violation_levels)
                    )
                    ...
                    fig1.savefig(os.path.join(testing_dir, f"cost distribution.png"), dpi=800)
                    plt.close(fig1)
                    fig2 = plt.figure()
                    plt.hist(results["costs"].cpu().numpy() - results["values"].cpu().numpy(), bins=200)
                    fig2.savefig(os.path.join(testing_dir, f"diff distribution.png"), dpi=800)
                    plt.close(fig1)
                ...
                logs["learned_volume"] = target_fraction(
                    ...
                    target_validator=ValueThresholdValidator(v_min=float("-inf"), v_max=0.0)
                    if dynamics.set_mode in ["reach", "reach_avoid"]
                    else ValueThresholdValidator(v_min=0.0, v_max=float("inf")),
                    num_samples=S,
                ...
                logs["recovered_volume"] = target_fraction(
                    ...
                    t=dataset.tMax,
                    sample_validator=ValueThresholdValidator(v_min=float("-inf"), v_max=float("inf")),
                    target_validator=ValueThresholdValidator(v_min=float("-inf"), v_max=delta_level)
                    if dynamics.set_mode in ["reach", "reach_avoid"]
                    else ValueThresholdValidator(v_min=delta_level, v_max=float("inf")),
                    num_samples=S,
                    batch_size=min(S, 1000000),
                ...
                results = scenario_optimization(
                    ...
                    set_type=set_type,
                    control_type=control_type,
                    scenario_batch_size=min(S, 100000),
                    sample_batch_size=10 * min(S, 10000),
                    sample_generator=SliceSampleGenerator(dynamics=dynamics, slices=[None] * dynamics.state_dim),
                    sample_validator=ValueThresholdValidator(v_min=float("-inf"), v_max=float("inf")),
                    violation_validator=ValueThresholdValidator(v_min=0.0, v_max=float("inf"))
                    if dynamics.set_mode in ["reach", "reach_avoid"]
            ...
            if data_step == "plot_basic_recovery":
                ...
                if plot_config["z_axis_idx"] == -1:
                    fig = self.plotSingleFig(state_test_range, plot_config, 512, 512, times, delta_level)
                else:
                    fig = self.plotMultipleFigs(state_test_range, plot_config, 512, 512, 5, times, delta_level)
                # plt.tight_layout()
                fig.savefig(os.path.join(testing_dir, f"basic_BRTs.png"), dpi=800)
                np.save(
                    os.path.join(testing_dir, f"volumes.npy"),
                    np.array(
                        [
                            float(logs["learned_volume"]),
                            float(logs["recovered_volume"]),
                            float(logs["theoretically_recoverable_volume"]),
                        ]
                    ),
                )
            ...
            if data_step == "plot_ND":
                ...
                for i in range(2 * len(times)):
                    ...
                    coords[:, 1:] = torch.tensor(
                        plot_config["state_slices"]
                    )  # initialized to zero (nothing else to set!)
                    ...
                    if learned_value.min() > 0:
                        ...
                        scaled_colors = np.vstack((matplotlib.colormaps[cmap_name](np.linspace(0.6, 1.0, 256))))
                        RdWhBl_vscaled = matplotlib.colors.LinearSegmentedColormap.from_list(
                            "RdWhBl_vscaled", scaled_colors
                        )
                        ...
                        scaled_colors = np.vstack((matplotlib.colormaps[cmap_name](np.linspace(0.0, 0.4, 256))))
                        RdWhBl_vscaled = matplotlib.colors.LinearSegmentedColormap.from_list(
                            "RdWhBl_vscaled", scaled_colors
                        )
                        ...
                        offset = 0
                        scaled_colors = np.vstack(
                            (
                                matplotlib.colormaps[cmap_name](np.linspace(0.0, 0.4, 256 - n_bins_high + offset)),
                                matplotlib.colormaps[cmap_name](np.linspace(0.6, 1.0, n_bins_high - offset)),
                            )
                        )
                        RdWhBl_vscaled = matplotlib.colors.LinearSegmentedColormap.from_list(
                            "RdWhBl_vscaled", scaled_colors
                    ...
                    if i >= len(times):
                        ...
                        cbar.set_ticks([learned_value.min(), 0.0, learned_value.max()])  # Define custom tick locations
                        cbar.set_ticklabels(
                            [f"{learned_value.min():1.1f}", "0", f"{learned_value.max():1.1f}"]
                        )  # Define custom tick labels

                        ## Plot Ground-Truth Zero-Level Contour

                        ax.contour(X1g, X2g, Vgt, [0.0], linewidths=4, alpha=0.7, colors="brown")
                        ax.contour(Xg, Yg, learned_value, [0.0], linewidths=1, alpha=1, colors="k", linestyles="--")
                        ...
                        ax.contour(
                            Xg, Yg, learned_value, zdir="z", offset=ax.get_zlim()[0], colors="k", levels=[0.0]
                        )  # cmap='bwr_r')
    ...
    def plotSingleFig(self, state_test_range, plot_config, x_resolution, y_resolution, times, delta_level=None):
        ...
        for i in range(len(times)):
            ...
            lx = (
                ...
                .cpu()
                .numpy()
                .reshape(x_resolution, y_resolution)
                .T
            )
            zero_contour = ax.contour(X, Y, BRT_img, levels=[0.0], colors="black", linewidths=2, linestyles="--")

            failure_set_contour = ax.contour(X, Y, lx, levels=[0.0], colors="saddlebrown", linewidths=2, linestyles="-")
            if delta_level is not None:
                delta_contour = ax.contour(
                    X, Y, BRT_img, levels=[delta_level], colors="black", linewidths=2, linestyles="-"
                )
        return fig

    def plotMultipleFigs(
        self, state_test_range, plot_config, x_resolution, y_resolution, z_resolution, times, delta_level=None
    ):
        x_min, x_max = state_test_range[plot_config["x_axis_idx"]]
        y_min, y_max = state_test_range[plot_config["y_axis_idx"]]
        ...
        for i in range(len(times)):
            for j in range(len(zs)):
                ...
                ax.set_title(
                    "t = %0.2f, %s = %0.2f" % (times[i], plot_config["state_labels"][plot_config["z_axis_idx"]], zs[j])
                )
                ...
                failure_set_contour = ax.contour(
                    X, Y, lx, levels=[0.0], colors="saddlebrown", linewidths=2, linestyles="-"
                )
    ...
    def adjust_rel_weight(self, losses, MPC_decay_scheme, epoch, epochs, model_input):
        if self.dataset.dynamics.deepReach_model in ["vanilla", "diff"] and losses["diff_constraint_hom"] > 0.01:
            ...
            losses["dirichlet"] = self.loss_weights["dirichlet"] * losses["dirichlet"]
            if (self.dataset.counter >= 1) and len(model_input["MPC_inputs"].shape) > 2:
                if MPC_decay_scheme == "exponential":
                    if self.MPC_importance_final <= self.MPC_importance_init:
                        self.mpc_importance_coef = self.MPC_importance_final * math.e ** (
                            math.log(self.MPC_importance_init / self.MPC_importance_final)
                            * (1 - (epoch - self.dataset.pretrain_iters) / (epochs - 1 - self.dataset.pretrain_iters))
                        )
                    else:
                        self.mpc_importance_coef = self.MPC_importance_final * math.e ** (
                            math.log(self.MPC_importance_init / self.MPC_importance_final)
                            * (epoch - self.dataset.pretrain_iters)
                            / (epochs - 1 - self.dataset.pretrain_iters)
                        )
                        self.mpc_importance_coef = (
                            self.MPC_importance_final + self.MPC_importance_init - self.mpc_importance_coef
                        )
                elif MPC_decay_scheme == "linear":
                    self.mpc_importance_coef = self.MPC_importance_init + (
                        self.MPC_importance_final - self.MPC_importance_init
                    ) * (epoch - self.dataset.pretrain_iters) / (epochs - self.dataset.pretrain_iters)
                else:
                    raise NotImplementedError
                ...
                self.loss_weights["mpc_loss"] = 0.9 * self.loss_weights[
                    "mpc_loss"
                ] + 0.1 * self.mpc_importance_coef * num / (den + 1e-16)

        elif (
            self.dataset.dynamics.deepReach_model == "exact"
            and (self.dataset.counter >= 1)
            and losses["mpc_loss"] > 1e-8
        ):
    ...
    def dataset_refinement(self, current_curriculum_time, epoch, epochs):
        ...
        if (
            ...
            self.dataset.policy = self.model
            self.dataset.use_terminal_MPC()
            self.dataset.generate_MPC_dataset(self.dataset.tMax, self.dataset.tMax, style="terminal")
    ...
    def plot_recovery_fig(self, dataset, dynamics, model, delta_level):
        ...
        for i in range(len(zs)):
            ...
            if dynamics.set_mode in ["reach", "reach_avoid"]:
                ...
                self.overlay_border(image, recovered_BRT, np.array([15, 223, 240]))
            ...
            ax.imshow(image.transpose(1, 0, 2), origin="lower", extent=(x_min, x_max, y_min, y_max))
            ...
            for y in range(ground_truth_brts.shape[1]):
                ...
                for neighbor in neighbors:
                    if (
                        neighbor[0] >= 0
                        and neighbor[1] >= 0
                        and neighbor[0] < ground_truth_brts.shape[0]
                        and neighbor[1] < ground_truth_brts.shape[1]
                    ):
            ...
            for y in range(set.shape[1]):
                ...
                for neighbor in neighbors:
                    if (
                        neighbor[0] >= 0
                        and neighbor[1] >= 0
                        and neighbor[0] < set.shape[0]
                        and neighbor[1] < set.shape[1]
                    ):
                        if not set[neighbor]:
