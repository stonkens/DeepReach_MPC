...
class MLPValidator(Validator):
    ...
    def validate(self, coords, values):
        model_results = self.model({"coords": self.dynamics.coord_to_input(coords.to(device))})
        inputs = torch.cat((coords[..., 1:].to(device), values[:, None].to(device)), dim=-1)
    ...
    def validate(self, coords, values):
        ...
        for i in range(len(self.o_levels) - 1):
            valids = torch.logical_or(
                valids,
                (outputs > self.o_levels[i])
                * (outputs <= self.o_levels[i + 1])
                * (values >= self.v_levels[i][0])
                * (values <= self.v_levels[i][1]),
            )
        return valids
...
def scenario_optimization(
    ...
    sample_generator,
    sample_validator,
    violation_validator=None,
    max_scenarios=None,
    max_samples=None,
    max_violations=None,
    tStart_generator=None,
    vf_eval_time=None,
    ...
    init_hams = torch.zeros(
        0,
    )
    mean_hams = torch.zeros(
        0,
    )
    mean_abs_hams = torch.zeros(
        0,
    )
    max_abs_hams = torch.zeros(
        0,
    )
    min_abs_hams = torch.zeros(
        0,
    )
    ...
    nums_valid_samples = []
    while True:
        if (max_scenarios and (num_scenarios >= max_scenarios)) or (
            max_violations and (num_violations >= max_violations)
        ):
            break
        batch_scenario_times = torch.zeros(
            scenario_batch_size,
        )
        batch_scenario_states = torch.zeros(scenario_batch_size, dynamics.state_dim)
        batch_scenario_values = torch.zeros(
            scenario_batch_size,
        )
        ...
        while num_collected_scenarios < scenario_batch_size:
            ...
            with torch.no_grad():
                ...
                batch_sample_values = dynamics.io_to_value(
                    batch_sample_model_results["model_in"].detach(),
                    batch_sample_model_results["model_out"].squeeze(dim=-1).detach(),
                )
            batch_valid_sample_idxs = (
                torch.where(sample_validator.validate(batch_sample_coords, batch_sample_values))[0].detach().cpu()
            )
            ...
            batch_scenario_states[start_idx:end_idx] = batch_sample_states[batch_valid_sample_idxs][
                : end_idx - start_idx
            ]
            batch_scenario_values[start_idx:end_idx] = batch_sample_values[batch_valid_sample_idxs][
                : end_idx - start_idx
            ]
        ...
        for k in tqdm(range(int((tMax - tMin) / dt)), desc="Trajectory Propagation", position=pbar_pos, leave=False):
            ...
            perceived_state = torch.clamp(
                state_trajs[:, k],
                torch.tensor(dynamics.state_test_range())[..., 0],
                torch.tensor(dynamics.state_test_range())[..., 1],
            )
            traj_coords = torch.cat((traj_times.unsqueeze(-1), perceived_state), dim=-1)
            ...
            if tStart_generator is not None:  # freeze states whose start time has not been reached yet
                ...
                state_trajs[is_unfrozen, k + 1] = dynamics.equivalent_wrapped_state(
                    state_trajs[is_unfrozen, k].to(device)
                    + dt
                    * dynamics.dsdt(
                        state_trajs[is_unfrozen, k].to(device),
                        ctrl_trajs[is_unfrozen, k].to(device),
                        dstb_trajs[is_unfrozen, k].to(device),
                    ).to(device)
                ).cpu()
            else:
                next_state_ = dynamics.equivalent_wrapped_state(
                    state_trajs[:, k].to(device)
                    + dt
                    * dynamics.dsdt(
                        state_trajs[:, k].to(device), ctrl_trajs[:, k].to(device), dstb_trajs[:, k].to(device)
                    )
                )
        ...
        if violation_validator is not None:
            num_new_violations = int(
                torch.sum(violation_validator.validate(batch_scenario_states, batch_scenario_costs))
            )
    ...
    precision = (
        TP.sum() / (TP.sum() + FP.sum()) if (TP.sum() + FP.sum()) > 0 else 0.0
    )  # if we predict unsafe, how often are we right?
    FNR = (
        FN.sum() / (FN.sum() + TP.sum()) if (FN.sum() + TP.sum()) > 0 else 0.0
    )  # of all unsafe cases, how many did we miss?
    recall = 1 - FNR  # of all unsafe cases, how many did we correctly identify as unsafe?
    FOR = (
        FN.sum() / (FN.sum() + TN.sum()) if (FN.sum() + TN.sum()) > 0 else 0.0
    )  # of all unsafe cases, how many did we identify as safe?
    ########
    TNR = (
        TN.sum() / (TN.sum() + FP.sum()) if (TN.sum() + FP.sum()) > 0 else 0.0
    )  # of all safe cases, how many did we identify as safe?
...
def target_fraction(model, dynamics, t, sample_validator, target_validator, num_samples, batch_size):
    with torch.no_grad():
        ...
        while len(states) < num_samples:
            ...
            batch_states = torch.zeros(batch_size, dynamics.state_dim)
            for dim in range(dynamics.state_dim):
                batch_states[:, dim].uniform_(*dynamics.state_verification_range()[dim])
                batch_states[:, dim] = batch_states[:, dim]
            batch_states = dynamics.equivalent_wrapped_state(batch_states)
            ...
            batch_values = dynamics.io_to_value(
                batch_model_results["model_in"], batch_model_results["model_out"].squeeze(dim=-1)
            ).detach()
