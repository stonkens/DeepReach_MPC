...
def get_mgrid(sidelen, dim=2):
    ...
    if dim == 2:
        ...
        pixel_coords = np.stack(np.mgrid[: sidelen[0], : sidelen[1], : sidelen[2]], axis=-1)[None, ...].astype(
            np.float32
        )
...
class ReachabilityDataset(Dataset):
    def __init__(
        ...
        if refinement_schedule is not None:
            self.refinement_schedule_index = 0  # Track current position in schedule
        ...
        self.add_disturbance_samples = add_disturbance_samples  # Only for robust reachability
        self.MPC_warm_start_only = MPC_warm_start_only  # Optional warm-start-only MPC mode
        self.warm_start_iterations = warm_start_iterations
        self.full_horizon_iterations = full_horizon_iterations
        if use_MPC and initiate_with_MPC:
            if MPC_data_path == "none":
                # initialize MPC dataset
                if refine_dataset:
                    if self.refinement_schedule is not None:
                        # Use first time in refinement schedule
                        initial_time = self.refinement_schedule[0]
                        self.generate_MPC_dataset(initial_time, 0.0)
                    else:
    ...
    def get_next_refinement_time(self, current_time):
        """Get the next refinement time based on schedule or constant interval."""
        if self.refinement_schedule is not None:
            # Find the next scheduled refinement time that is greater than current_time
            for time in self.refinement_schedule:
                if time > current_time:
                    return time
            return None  # No more refinements scheduled
        else:
    ...
    def sample_init_state(self):
        ...
        while MPC_states.shape[0] < self.MPC_batch_size:
            MPC_states_ = torch.zeros(self.MPC_batch_size, self.dynamics.state_dim).uniform_(-1, 1)
        ...
        MPC_states = self.dynamics.input_to_coord(torch.cat((torch.ones(MPC_states.shape[0], 1), MPC_states), dim=-1))[
            ..., 1:
        ]

        if self.num_target_samples > 0:
            num_mpc_target_samples = min(int(self.MPC_batch_size / 5), self.num_target_samples)
            target_state_samples = self.dynamics.sample_target_state(num_mpc_target_samples)
    ...
    def collect_MPC_io(self, mpc_fn, T, t, style="random"):
        ...
        outputs = mpc_fn.get_batch_data(
            MPC_states.to(device), T, self.policy, t=t
        )  # Make sure to generate at least one batch of data at T, so we have "look-ahead" MPC labels for deepreach
        MPC_inputs = outputs[2]
        MPC_values = outputs[3]
        for i in tqdm(range(self.num_MPC_batches - 1)):
            ...
            # t_max=self.tMax
            outputs = mpc_fn.get_batch_data(MPC_states.to(device), t_max, self.policy, t=t)
            MPC_inputs_ = outputs[2]
            MPC_values_ = outputs[3]
    ...
    def generate_MPC_dataset(self, T, t, style="random"):
        ...
        del mpc_fn  # free it to get some memory
        gc.collect()
        empty_cache()
    ...
    def __getitem__(self, idx):
        ...
        while model_states_normed.shape[0] < self.numpoints:
            model_states_normed_ = torch.zeros(self.numpoints, self.dynamics.state_dim).uniform_(-1, 1)
        ...
        if self.num_target_samples > 0:
            ...
            model_states_normed[-self.num_target_samples :] = self.dynamics.coord_to_input(
        ...
        boundary_values = self.dynamics.boundary_fn(self.dynamics.input_to_coord(model_inputs)[..., 1:])
        if self.dynamics.loss_type == "brat_hjivi":
            reach_values = self.dynamics.reach_fn(self.dynamics.input_to_coord(model_inputs)[..., 1:])
            avoid_values = self.dynamics.avoid_fn(self.dynamics.input_to_coord(model_inputs)[..., 1:])
...
class RobustReachabilityDataset(ReachabilityDataset):
    ...
    def generate_MPC_dataset(self, T, t, style="random"):
        ...
        if self.add_disturbance_samples:
            ...
            # concatenate control and disturbance inputs
        ...
        del mpc_control_fn, mpc_disturbance_fn  # free it to get some memory
        gc.collect()
        empty_cache()
