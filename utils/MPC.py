...
class MPC:
    ...
    def get_batch_data(self, initial_condition_tensor, T, policy=None, t=0.0):
        ...
        if self.dynamics_.set_mode in ["avoid", "reach"]:
            ...
            state_trajs, avoid_values, reach_values, num_iters = self.get_opt_trajs(initial_condition_tensor, policy, t)
    ...
    def get_opt_trajs(self, initial_condition_tensor, policy=None, t=0.0):
        ...
        if self.style == "direct":
            ...
            state_trajs = torch.zeros((self.batch_size, num_iters + 1, self.dynamics_.state_dim)).to(
                self.device
            )  # A*H*D
            ...
            for i in tqdm(range(int(num_iters / self.receding_horizon))):
                best_controls, _ = self.get_control(state_trajs[:, i, :])
    ...
    def warm_start_with_policy(self, initial_condition_tensor, policy=None, t_remaining=None):
        ...
        if self.incremental_horizon > 0:
            ...
            if t_remaining > 0.0:
                ...
                state_trajs_clamped = torch.clamp(
                    state_trajs_H[:, :, -1, :],
                ...
                traj_policy_results = policy({"coords": self.dynamics_.coord_to_input(traj_coords.to(self.device))})
                terminal_values = self.dynamics_.io_to_value(
                    traj_policy_results["model_in"].detach(), traj_policy_results["model_out"].squeeze(dim=-1).detach()
                )
                if self.incremental_horizon > 0:
    ...
    def get_control(self, initial_condition_tensor, num_iterative_refinement=1, policy=None, t_remaining=None):
        ...
        if self.style == "direct":
            ...
            for i in range(num_iterative_refinement + 1 - self.num_effective_horizon_refinement):
                ...
                _, best_traj, best_costs = self.update_control_tensor(state_trajs, permuted_controls)
            return self.control_tensors, best_traj
        elif self.style == "receding":
            # initial_condition_tensor: A*D
            state_trajs, permuted_controls = self.rollout_dynamics(
                initial_condition_tensor,
                start_iter=self.receiding_start,
                rollout_horizon=self.horizon - self.receiding_start,
            )

            current_controls, best_traj, _ = self.update_control_tensor(state_trajs, permuted_controls)
    ...
    def rollout_with_policy(self, initial_condition_tensor, policy, policy_horizon, policy_start_iter=0):
        ...
        for k in range(policy_horizon):
            traj_coords = torch.cat((traj_times, state_trajs_clamped[:, k, :]), dim=-1)
            traj_policy_results = policy({"coords": self.dynamics_.coord_to_input(traj_coords.to(self.device))})
    ...
    def update_control_tensor(self, state_trajs, permuted_controls):
        ...
        if self.mode == "MPC":
            ...
            num = torch.sum(
                exp_terms[:, :, None, None].repeat(1, 1, self.horizon, self.dynamics_.control_dim) * permuted_controls,
                dim=1,
            )  # A * H * D_u
    ...
    def rollout_nominal_trajs(self, initial_state_tensor):
        ...
        """
        # rollout trajs
        state_trajs = torch.zeros((self.batch_size, self.horizon + 1, self.dynamics_.state_dim)).to(
            self.device
        )  # A * H * D
        state_trajs[:, 0, :] = initial_state_tensor * 1.0  # A * D

        for k in range(self.horizon):
            state_trajs[:, k + 1, :] = self.get_next_step_state(state_trajs[:, k, :], self.control_tensors[:, k, :])
        return state_trajs
    ...
    def rollout_dynamics(self, initial_state_tensor, start_iter, rollout_horizon, eps_var_factor=1):
        ...
        if self.sample_mode == "gaussian":
            epsilon_tensor = (
                torch.randn(self.batch_size, self.num_samples, rollout_horizon, self.dynamics_.control_dim).to(
                    self.device
                )
                * torch.sqrt(self.dynamics_.eps_var)
                * eps_var_factor
            )  # A * N * H * D_u
            ...
            permuted_controls = torch.sign(
                torch.empty(self.batch_size, self.num_samples, rollout_horizon, self.dynamics_.control_dim).uniform_(
                    -1, 1
                )
            ).to(self.device)
            # always include the nominal trajectory
            permuted_controls[:, 0, ...] = self.control_tensors[:, start_iter : start_iter + rollout_horizon, :] * 1.0

        # clamp control
        permuted_controls = torch.clamp(
            permuted_controls, self.dynamics_.control_range_[..., 0], self.dynamics_.control_range_[..., 1]
        )

        # rollout trajs
        state_trajs = torch.zeros(
            (self.batch_size, self.num_samples, rollout_horizon + 1, self.dynamics_.state_dim)
        ).to(self.device)  # A * N * H * D
