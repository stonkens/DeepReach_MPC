    ...
    def brt_hjivi_loss(
        ...
        if MPC_loss_type == "l1":
            ...
            if use_MPC_terminal_loss:
                ...
                if dynamics.set_mode == "avoid":
                    unsafe_cost_safe_value_indeces = (
                        torch.argwhere(torch.logical_and(MPC_labels.squeeze(0) < 0.0, MPC_values.squeeze(0) >= 0.0))
                        .detach()
                        .squeeze(-1)
                    )
                else:
                ...
                mpc_loss += (
                    torch.abs(
                        MPC_values[:, unsafe_cost_safe_value_indeces] - MPC_labels[:, unsafe_cost_safe_value_indeces]
                    )
            ...
            if use_MPC_terminal_loss:
                ...
                mpc_loss += (
                    torch.pow(
                        MPC_values[:, unsafe_cost_safe_value_indeces] - MPC_labels[:, unsafe_cost_safe_value_indeces], 2
                    )
                    * torch.abs(MPC_values[:, unsafe_cost_safe_value_indeces])
                ).sum() * MPC_finetune_lambda
        else:
